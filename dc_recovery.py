...
def dct_transform(image, dc_free=False):
    """
...
def recover_from_dct(dct_coefs):
...
def enhance_dc_recovery(image_path):
    ...
    dct_coefs_corner = dct_transform(image, dc_free=False)

    dct_preds = dc_prediction_1(dct_coefs, dct_coefs_corner[0, 0, 0, 0])
    dct_preds += dc_prediction_2(dct_coefs, dct_coefs_corner[0, h_n - 1, 0, 0])
    dct_preds += dc_prediction_3(dct_coefs, dct_coefs_corner[w_n - 1, 0, 0, 0])
    dct_preds += dc_prediction_4(dct_coefs, dct_coefs_corner[w_n - 1, h_n - 1, 0, 0])
    dct_preds /= 4.
    image_rec = recover_from_dct(dct_preds)
...
def pre_work(image_path):
    ...
    image = cv2.resize(image_gray, (w, h, ))
...
if __name__ == '__main__':
    ...
    folder_path = current_path + "/all_trainset"
    enhance_output_folder = current_path + "/backward_train_rec"
    os.makedirs(enhance_output_folder, exist_ok=True)

    for i in tqdm(range(1900)):
        image_path = os.path.join(folder_path, f"{2100+i}.jpg")
        # Assuming dc_recovery returns a tuple with the original image and the recovered image
        enhance_image_rec = enhance_dc_recovery(image_path)
        # Save the recovered image to the output folder
        output_path = os.path.join(enhance_output_folder, f"{2100+i}_rec.jpg")
        cv2.imwrite(output_path, enhance_image_rec)
